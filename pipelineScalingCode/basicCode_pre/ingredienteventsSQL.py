...
def row_to_prose(row):
    ...
    if subject_id and starttime and endtime:
        sentences.append(f"Patient {subject_id} received medication from {starttime} to {endtime}.")
    # IDs
    if hadm_id:
    ...
    if status and ("rate" in status) and originalrate:
        ...
        sentences.append(f"The drug administration {status}.")
    
    return " ".join(sentences)
    
# CSV path
file_path = os.path.join("mimic-iv-demo", "mimic-iv-clinical-database-demo-2.2", "icu", "ingredientevents.csv")
